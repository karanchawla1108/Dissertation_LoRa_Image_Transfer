...
# -- UPDATED: ImprovedVAE to match new trained model --
LATENT_DIM = 64  # was 32 — must match training
...
BASE_DIR = '/home/ysj/Result for the Image improved'
...
# -- UPDATED: 6 packets for 256 bytes (64 floats x 4 bytes) --
def receive_packets(num_packets=6, timeout=999999):
    received = {}
    t_start = time.time()
    p_before = ina.power
    ...
    for i in range(num_packets):
        if i in received:
            ...
            # Each packet is 48 bytes except last which is 256 - (5*48) = 16 bytes
...
def save_log(run_folder, packets_received, rx_time, rx_power, decode_time, decode_power, total_time, ssim_score):
    ...
    with open(log_path, 'w') as f:
        f.write("VAE LoRa Transmission Results (Improved Model)\n")
        f.write("===============================================\n")
        f.write("Packets received:  " + str(packets_received) + "/6\n")
...
print("Packets received:  " + str(packets_received) + "/6")
print("RX time:           " + str(round(rx_time, 1)) + "ms")
